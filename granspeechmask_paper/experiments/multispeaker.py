...
from granspeechmask_paper.evaluation.accuracy_metrics import run_accuracy_metrics
from granspeechmask_paper.experiments.common import TemplateSettingExp, multispeakers_list
...
exp_name = "SpeechConcealerExpMultispeaker_20260529"
...
HOST_PATHS = {
    'pc-ls2n-lagrange': ('../speechConcealerExp/', '../SOS-2SP/'),
    'serveur-ls2n-cpu1': ('../speechConcealerExp/', '../SOS-2SP/'),
    'serveur-ls2n-gpu3': ('../speechConcealerExp/', '../SOS-2SP/'),
    # for ssd from pc-lagrange
    'po-tailleur-840g8': ('/media/user/MT-SSD-3/0-PROJETS_INFO/Post-doc/speechConcealerExp/', '/media/user/EXTbackup/speech_dataset/SOS-2SP/'),
}
DEFAULT_PATHS = (
    '../SpeechConcealer-data/',
    '/media/user/MT-SSD-3/0-PROJETS_INFO/Post-doc/datasets/SOS-2SP/',
)
...
experiment.set_path('accuracy', EXP_DIR+"/"+experiment.name+'/accuracy/', force=True)
...
experiment.add_plan('simplelist',
  ...
  speaker = multispeakers_list,
  max_len_strat = ["queue", "diversity_marginal_gain"],
  fe = ["logmel", "mfcc"],
  dw = [50, 300], # in ms
...
class SettingExp(TemplateSettingExp):
    def __init__(self, setting, experiment, dataset_dir, config_dir):
        ...
        num_calib_files = 2
        self.speakers_list = multispeakers_list
        self.calib_audio_files = self.audio_files[:num_calib_files] 
        self.eval_audio_files = self.audio_files[num_calib_files:]
        self.accuracy_dir = experiment.path.accuracy 
        self.exptype = self._normalize(getattr(setting, 'exptype', None))
        self.turns_path = f"{dataset_dir}/turns/{self.speaker}/"
...
def step(setting, experiment):
    ...
    setting_exp = SettingExp(setting, experiment, dataset_dir=DATASET_DIR, config_dir=CONFIG_DIR)
    ...
    if setting_exp.step == "generate":
        ...
        # Accuracy (did the concealer pick the right speaker?) only makes
        # sense for models that actually select a speaker-matched concealer.
        # oracle has no concealment at all, and whitenoise is synthetic noise
        # with no speaker selection (self.streamconfig is also never set for
        # either, since TemplateSettingExp only builds it for real concealer
        # models) — so skip accuracy for both.
        if setting_exp.model not in ["oracle", "oracle3db", "whitenoise"]:
            run_accuracy_metrics(setting_exp)
